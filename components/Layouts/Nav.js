...
export const Nav = () => {
  ...
  return (
    <nav className="header-navigation top-0 w-full left-0 dark:bg-black px-2 py-3 shadow-sm flex items-center z-50 sticky">
      <button
        onClick={toggleNav}
      ...
      <div className="navBar-content ms-3 w-full ps-4  box-border">
        ...
        <form className="group  relative flex items-center" onSubmit={submit}>
          ...
          <input
            className="bg-transparent w-full focus:outline-none appearance-none border-transparent text-sm leading-6 text-slate-20 placeholder-slate-500 focus:border-slate-800  rounded-md py-2 pl-10"
            type="search"
        ...
        {/* =======.Result Container ======= */}
        <div
          className={`search-result-container w-full box-border bg-dark-200 min-h-full w shadow-md right-0 max-w-7xl absolute transition-all top-20 p-5 rounded-md  sm:max-w-full ${
        ...
        >
          ...
          <div
            className={`search-history ${
              input.trim().length === 0 ? "visible" : "hidden"
            }`}
          >
            ...
            <div className="history-items-container mt-3">
              {history.map((item) => (
                <div
                  className="history-item rounded my-2 bg-slate-800 inline-block px-3 py-1 mr-3"
                  key={item}
                >
                  <span
