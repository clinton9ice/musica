...
export const Playlist = ({ playlist }) => {
  ...
  const {selectTrack, playing, pause, play, player } = useContext(musicContext)
      ...
      {Array.isArray(playlists) &&
        playlists.map((item) => {
          return (
            <li
              key={item.duration}
              className={`flex cursor flex-wrap lg:flex-nowrap justify-between pl-3 my-5 py-4 items-center dark:bg-dark-300 bg-warmGray-200 rounded-2xl hover:-translate-y-3 transition-all ease-linear lg:pr-6 w-full ${item.id == playing?.id && playing?.category == item.category ? 'border border-amber-300 animate-pulse' : ''}`}
            >
              <div className="flex items-center justify-between w-full md:w-auto pr-4 image_section gap-4 mr-4">
                <div className="img-thumbnail relative w-20 h-20 lg:w-12 lg:h-12 rounded-md overflow-hidden dark:bg-slate-700 bg-warmGray-400">
                  <Image src={item.cover} layout="fill" unoptimized className="object-cover" />
                </div>
